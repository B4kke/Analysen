...
function MediaMentionCard({ mention, investigationId }: { mention: MediaMention; investigationId: string }) {
  ...
  const imageUrl = mention.image_embeddable && mention.image_document_id
    ? `${API_BASE}/api/v1/investigations/${encodeURIComponent(investigationId)}/media/image/${encodeURIComponent(mention.image_document_id)}`
    : null;
  return (
    <article className="coverage-row" aria-label="Medienevnt">
      <div>
        ...
        {imageUrl ? <img className="mention-image" src={imageUrl} alt={mention.headline || mention.publication || "Lagret artikkelutsnitt"} loading="lazy" /> : null}
