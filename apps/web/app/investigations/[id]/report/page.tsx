...
export default function ReportPage() {
  ...
  useEffect(() => { fetch(`${API_BASE}/api/v1/investigations/${id}/report/sections`).then(async (response) => { const payload = await response.json().catch(() => null); if (!response.ok) throw new Error(payload?.detail || "Fant ikke rapporten."); setSections(payload); }).catch((caught) => setError(caught instanceof Error ? caught.message : "Kunne ikke laste rapporten.")); }, [id]);
